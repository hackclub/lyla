...
async function fetchSnippet(channel, threadTs) {
  try {
    const resp = await botClient.conversations.replies({ channel, ts: threadTs, limit: 1, inclusive: true });
...
export async function mergeCase(fromCaseNumber, toCaseNumber) {
  await db.transaction(async (tx) => {
    ...
    if (toInsert.length > 0) {
      await tx.insert(caseAssignees).values(
        toInsert.map((a) => ({ ...a, caseNumber: toCaseNumber }))
      );
    }
    ...
    await tx
      .update(cases)
      .set({ status: "merged", resolvedAt: Date.now(), resolutionKind: "merged", mergedInto: toCaseNumber })
